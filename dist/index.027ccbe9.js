...
})({"p4COA":[function(require,module,exports,__globalThis) {
...
module.bundle.HMR_BUNDLE_ID = "52f8c156027ccbe9";
"use strict";
...
},{}],"zjufB":[function(require,module,exports,__globalThis) {
...
async function getApiKey() {
    ...
    // Using Gemini AI model; adjust the model name as necessary
    model = genAI.getGenerativeModel({
        model: "gemini-1.5-flash"
    });
}
// Call getApiKey on page load
getApiKey();
// --- Local Book Data & Functions ---
// For demonstration purposes, we'll use an in-memory array.
// In your production app, these should call Firestore.
let books = []; // This array will store your book objects
function addBook(title, author, genre, rating) {
    ...
    books.push(newBook);
    updateBookListUI(); // Refresh the book list display
    return true;
}
function editBookTitle(oldTitle, newTitle) {
    const book = books.find((b)=>b.title.toLowerCase() === oldTitle.toLowerCase());
    if (book) {
        book.title = newTitle;
        updateBookListUI();
        return true;
...
// Function to update the Book List UI
function updateBookListUI() {
    ...
    books.forEach((book)=>{
        ...
        li.textContent = `${book.title} by ${book.author} (${book.genre}) - Rating: ${book.rating}`;
        bookList.appendChild(li);
    });
}
// --- Chatbot UI & Interaction ---
// Wait for DOMContentLoaded to hook up the chat UI
document.addEventListener("DOMContentLoaded", ()=>{
    ...
    async function askChatBot(userInput) {
        // If model is not ready, inform the user
    ...
    // --- Handle Local Book Commands ---
    // Returns true if the command was handled locally.
    function handleLocalCommands(userInput) {
        const lowerInput = userInput.toLowerCase();
        // Command: Add Book
        // Expected format: "add book Title by Author, Genre, Rating"
        if (lowerInput.startsWith("add book")) {
            const details = userInput.slice(8).trim(); // Remove "add book"
            // Expecting format "Title by Author, Genre, Rating"
            ...
            if (parts.length >= 2) {
                ...
                } else appendMessage("Please provide book details as: add book Title by Author, Genre, Rating", "bot");
            } else appendMessage("Please provide complete details: add book Title by Author, Genre, Rating", "bot");
            return true;
        }
        // Command: Edit Book (for renaming only in this simple example)
        // Format: "edit book Old Title to New Title"
        if (lowerInput.startsWith("edit book")) {
            const details = userInput.slice(9).trim(); // Remove "edit book"
            const [oldTitle, newTitle] = details.split(" to ");
            if (oldTitle && newTitle) {
                const success = editBookTitle(oldTitle.trim(), newTitle.trim());
                if (success) appendMessage(`Renamed book "${oldTitle.trim()}" to "${newTitle.trim()}"`, "bot");
                else appendMessage(`Book "${oldTitle.trim()}" not found.`, "bot");
            } else appendMessage('Use: "edit book Old Title to New Title"', "bot");
            return true;
        }
        // Command: Delete Book
        // Format: "delete book Title"
        if (lowerInput.startsWith("delete book")) {
            const title = userInput.slice(11).trim(); // Remove "delete book"
            if (title) {
                const success = deleteBookByTitle(title);
        ...
        // Command: Suggest Book
        // Format: "suggest book" or "suggest a book"
        if (lowerInput.includes("suggest") && lowerInput.includes("book")) {
            const suggestion = getLocalSuggestion();
            if (suggestion) appendMessage(`How about "${suggestion.title}" by ${suggestion.author}?`, "bot");
            else appendMessage("No books available to suggest.", "bot");
            return true;
        }
        return false;
    }
    ...
    sendBtn.addEventListener("click", async ()=>{
        ...
        chatInput.value = "";
        // If a local command is recognized, do not call the AI
        if (handleLocalCommands(userInput)) return;
        // Otherwise, get a response from the AI
...
},{"@google/generative-ai":"gKJrW","firebase/firestore":"8A4BC","./firebase.js":"eUV3w"}],"gKJrW":[function(require,module,exports,__globalThis) {
/**
...
},{"@parcel/transformer-js/src/esmodule-helpers.js":"gkKU3"}]},["p4COA","zjufB"], "zjufB", "parcelRequire94c2")

//# sourceMappingURL=index.027ccbe9.js.map
